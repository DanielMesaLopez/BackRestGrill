...
const getOferta = (request, response) => {
    connection.query("SELECT * FROM oferta where estado=1", 
    (error, results) => {
        if(error)
            throw error;
...
const putOferta = (request, response) => {
    const {id_oferta, id_productos, nombre_oferta, descripcion, estado, fecha_inicio, fecha_fin, descuento} = request.body;
    connection.query("UPDATE oferta set id_productos= ?, nombre_oferta= ?, descripcion= ?, estado= ?, fecha_inicio= ?, fecha_fin= ?, descuento= ?  where id_oferta=?",
    [id_productos, nombre_oferta, descripcion, estado, fecha_inicio, fecha_fin, descuento, id_oferta],
    (error, results) => {
       if(error)
          throw error;
