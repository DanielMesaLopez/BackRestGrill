...
const getCategorias = (request, response) => {
    connection.query("SELECT * FROM categorias where estado=2", 
    (error, results) => {
        if(error)
            throw error;
...
const postCategorias = (request, response) => {
    ...
    connection.query("INSERT INTO categoria(id_categoria, nombre_categoria, descripcion, estado) VALUES (?, ?, ?, ?)",
...
const putCategorias = (request, response) => {
    const {id_categoria, nombre_categoria, descripcion, estado} = request.body;
    connection.query("UPDATE categoria set nombre_categoria= ?, descripcion= ?, estado= ?  where id_categoria=?",
...
const delCategorias= (request, response) => {
    ...
    connection.query("update  categoria set estado=0 where id_categoria = ?",
    [id_productos],
    (error, results) => {
        if(error)
            throw error;
